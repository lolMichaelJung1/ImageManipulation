...
public class Main{
  ...
  public static void main(String[] args){
    ...
    if(choice == 6){
      System.out.println("Enter rgb change values: ");
      int r = sc.nextInt();
      int g = sc.nextInt();
      int b = sc.nextInt();
      twoDToImage(colorFilter(imageData, r,g,b), file);     
    }
    ...
    if(choice == 11){
      twoDToImage(columnFlip(imageData), file);         
    }
      //trycatch for InputMismatchException and scanner flush (reset the scanner before)
    if(choice == 12){
      System.out.println("Enter density number: ");
      String s = sc.nextLine();
      while(!isInt(s)){
        System.out.println("Enter density: ");
        s = sc.nextLine();
    }
      int density = Integer.parseInt(s);
